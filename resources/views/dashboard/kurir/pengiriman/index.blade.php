                                    ...
                                    <table class="table table-hover table-bordered mb-0">
                                        ...
                                        <tbody>
                                            @foreach ($pengiriman as $index => $item)
                                                <tr>
                                                    <td>{{ $index + 1 }}</td>
                                                    <td>{{ $item->pemesanan_produk->pemesanan->user->name }}</td>
                                                    <td>{{ $item->pemesanan_produk->produk->nama }}</td>
                                                    <td>{{ $item->pemesanan_produk->qty_produk }}</td>
                                                    <td>
                                                        <span
                                                            class="badge 
        @if ($item->status_pengiriman == 'BelumBayar') badge-danger
        @elseif($item->status_pengiriman == 'Dikemas') badge-warning
        @elseif($item->status_pengiriman == 'Selesai') badge-success @endif">
                                                            {{ $item->status_pengiriman }}
                                                        </span>
                                                    </td>
                                                    {{-- <td>{{ number_format($item->pemesanan_produk->harga, 0, ',', '.') }}</td> --}}
